...
export function QuestionCard({ q, index, total, selected, onSelect, showFeedback, showAtEnd, correct }: Props) {
  ...
  return <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 space-y-4">
    ...
    <div className="space-y-3">
      {q.shuffledOptions.map((o)=><div key={o} className="space-y-2"><button disabled={isLocked} onClick={()=>onSelect(o)} className={`block w-full text-left rounded-2xl border px-4 py-3.5 sm:px-5 sm:py-4 text-sm sm:text-base transition-all duration-200 min-h-12 focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-200 ${selected===o?'border-blue-500 bg-blue-50 text-blue-900':'border-slate-200 bg-white text-slate-800 hover:border-blue-300 hover:bg-slate-50'} ${shouldShowPerOptionFeedback && o===q.correctAnswer?'bg-green-100 border-green-500 text-green-900':''} ${shouldShowPerOptionFeedback && selected===o && selected!==q.correctAnswer?'bg-red-100 border-red-500 text-red-900':''} ${isLocked?'cursor-not-allowed opacity-95':'active:scale-[0.99]'}`}>{o}</button>
        {shouldShowPerOptionFeedback && <div className="px-2 sm:px-3 pb-1">
          <p className={`text-xs sm:text-sm font-semibold ${o===q.correctAnswer?'text-green-700':'text-red-700'}`}>{o===q.correctAnswer?'✅ Respuesta correcta':'❌ Respuesta incorrecta'}</p>
          <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">{o===q.correctAnswer?(q.explanation || 'Esta es la respuesta correcta.'):(q.optionExplanations?.[o] || 'Esta opción no es correcta para esta pregunta.')}</p>
        </div>}
      </div>)}
    </div>
